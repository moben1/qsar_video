...
def scene_pipeline(scene: Scene):
    ...
    workflow_title = Tex("Flux de travail", font_size=34, color=BLUE)
    workflow_title.to_edge(UP)
    scene.play(FadeIn(workflow_title))

    colors = [BLUE, YELLOW, PINK, GREEN]

    steps = ["Génération de données", "Prétraitement de données", "Entrainement du modèle", "Évaluation et validation"]
    ...
    arrows = [Arrow(boxes[i].get_right(), boxes[i + 1].get_left(), buff=0.5, stroke_width=20, stroke_color=YELLOW)
              for
              i in range(3)]

    for idx, (box, step) in enumerate(zip(boxes, steps)):
        scene.play(Create(box), run_time=0.5)
        step_text = Text(step, color=BLACK, font_size=14).move_to(box.get_center())
